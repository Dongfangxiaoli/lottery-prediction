"""
大乐透历史开奖数据获取器
数据源: 500.com HTML 页面 (sporttery.cn API 已于 2026 年返回 567)
"""
import os
import time
import requests
...
CSV_PATH = os.path.join(DATA_DIR, "dlt_history.csv")
...
def _parse_500_html(html: str) -> pd.DataFrame:
    """解析 500.com DLT 历史数据 HTML 表格"""
    soup = BeautifulSoup(html, "html.parser")

    # 找历史数据表格：class 含 'chart' 或 id='history'
    ...
    for tr in rows:
        ...
        if len(cells) < 8:
            continue
        # 500.com DLT 表格列：期号, 前1-5, 后1-2, 销售额, ...
        try:
            ...
            fronts = []
            for j in range(1, 6):
                fronts.append(int(cells[j].get_text(strip=True)))
            backs = []
            for j in range(6, 8):
                backs.append(int(cells[j].get_text(strip=True)))
            ...
            if not date_str:
                # 尝试第8列
                if len(cells) > 8:
                    date_str = cells[8].get_text(strip=True)

            # 销售额可能在倒数第2列
            sales = 0
            for j in range(len(cells) - 1, 7, -1):
                t = cells[j].get_text(strip=True).replace(",", "").replace("元", "")
                try:
                    sales = int(t)
                    break
                except ValueError:
...
def fetch_all(progress_callback=None) -> pd.DataFrame:
    """一次性拉取 500.com 全部大乐透历史数据（单页 HTML，无需分页）"""
    if progress_callback:
        progress_callback(0.2, "正在从 500.com 获取大乐透全部历史数据...")
